...
public class hwshinzufuegenActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ab.setTitle("Hardwaresystem hinzufügen");                       //to-do String Value

        //Fragment-Setup
        hwshinzufuegenView AV = (hwshinzufuegenView) getSupportFragmentManager().findFragmentById(R.id.contentFrameKarte);
        //Falls AV nicht vorhanden ist, dann erstelle das View-Fragment und füge dies der Aktivität hinzu
        if (AV == null) {
            AV = hwshinzufuegenView.newInstance();
            Generell.addFragmentToActivity(getSupportFragmentManager(), AV, R.id.contentFrameKarte);
        }

        mPresenter = new hwshinzufuegenPresenter(AV,mBenutzername);
    }
